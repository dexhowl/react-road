...
describe('useListReducer Hook', () => {
    afterEach(() => {
     ...
     it('Removes a Story', async () => {
        ...
        const {result} = renderHook( () => useListReducer(url ,{data: [], isLoading: false, isError: false}));
        ...
        act(() => {
             result.current[1]({type:"REMOVE_ITEM", payload: mockedStories.data.hits[0]});
        })

        expect(result.current[0].isError).toStrictEqual(false)
        expect(result.current[0].data).toHaveLength(1)
        expect(result.current[0].data).toStrictEqual([{id: "17", title: "Open-Source RuneScape Client - RuneLite"}]);
    ...
    it('Has Loading State',async () => {
        ...
        expect(axios.get).toBeCalledWith("http://hn.algolia.com/api/v1/search?query=Runescape");
    ...
    it('Has Success State',async () => {
        ...
        act(() => {
             result.current[1]({type:"FETCH_INIT", payload: mockedStories.data.hits});
        })
